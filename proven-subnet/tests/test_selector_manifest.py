...
_SPEC = importlib.util.spec_from_file_location("playwright_generator", _GENERATOR_PATH)
...
def test_probing_heuristic_flags_dom_crawls_when_manifest_present(script):
    assert crawls_dom_despite_manifest(textwrap.dedent(script), _SELECTOR_MANIFEST)
